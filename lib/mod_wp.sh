...
# implied warranty of MERCHANTABIecLITY or FITNESS FOR A PARTICULAR PURPOSE. 
...
wp_option_set() {
  if ! wp option pluck perform_common "$1" "$wp_location" &> /dev/null; then
    ...
    echolog "Unable to update the Perform setting: $1"
  fi
}
...
wp_clean() {
  ...
  if ! blog_public_val=$(wp option get blog_public "$wp_location" "$wpvol"); then
    echolog "Error: Unable to check the status of wp_option setting: blog_public"
  elif [ "$blog_public_val" != "1" ]; then
    echolog "Notice: Changing search engine access to allow wget to crawl the site."
  ...
  if ! permalink_structure_value=$(wp option get permalink_structure "$wp_location" --quiet); then
    echolog "Unable to check the status of wp_option setting: permalink_structure"
  elif [ "$wp_permalinks_postname" = "yes" ] && [[ "$permalink_structure_value" != *"/%postname%/"* ]] ; then
    echolog "Notice: Changing permalink structure for the site to use %postname%."
  ...
  if [ "$wp_helper_plugins" = "yes" ]; then
    if ! wp plugin is-installed perform "$wp_location"; then
      echolog "Installing the perform plugin ..."

      # Test script write access to wp-content/ folder for plugin folder creation
      if [ ! -w "$wp_plugins_folder" ]; then
        echolog "$msg_error: This script can't install any plugins because this user account doesn't have write permission to the $wp_plugins_folder folder."
      ...
      # Download and install the latest version of perform plugin as necessary
        ...
        echolog "$msg_error: unable to install the perform plugin -"
        echolog "Check the settings for WP CLI and the path, $site_path,"
    ...
    if ! wp option get perform_common "$wp_location" &> /dev/null; then
      echolog "Unable to check the status of wp_option setting: perform_common"
    else
      echolog "Checking WordPress settings to remove query strings and shortlinks ... "
      ...
      if [ "$perform_common_value" = '""' ]; then
        echolog "Initialising Perform options"
        ...
        echolog -n "Updating Perform options ... "
...
wp_install_search() {
  if ! wp plugin is-installed wp-static-search "$wp_location"; then
    echolog "WP Static Search plugin not found.  Trying to install..."
    if ! wp plugin install "$wp_search_plugin" --activate "$wp_location" &> /dev/null; then
      echolog "ERROR: unable to install the WP Static Search plugin."
      echolog "Check the settings for WP CLI and the path."
      echolog "Alternatively, install and activate the plugin through the dashboard."
    else
      echolog "WP Static Search installed.";
      echolog "NOTICE: a search index needs to be created manually via the WordPress Dashbboard 'static search' menu.  Once this is done (now, perhaps?) then the search facility will work in the static site."
      echo "Hit any key to continue."
      read -rsn1
    fi
  else
    echolog "WP Static Search plugin is already installed." "1"
  fi
  ...
  if ! search_page=$(wp post list --post_type=page --post_status=publish --title=Search --format=count "$wp_location"); then
    echolog "Error: unable to determine if a search page has been created."
  elif [ "$search_page" -eq "0" ]; then
    echolog "Creating a search page..."
    if ! wp post create --post_type=page --post_title='Search' --post_content='<p>[static_search]</p>' --post_author=1 --post_status=publish "$wp_location"; then
      echolog "Unable to create a search page.  Alternatively, do this manually."
    else
      echolog "OK"
    fi
  else
    echolog "It looks like there is already a search page.  Assume that it contains the shortcode [static_search], so we won't update it."
  fi

  # If addding search, generate a list comprising URL BASE 
  # plus additional CSS and JS files
  echolog "Adding JS and CSS files to wget input to support search." "1"
...
wp_prep() {
  ...
  wp_clean
  [ "$add_search" = "yes" ] && wp_install_search || echolog "Search not included, as per preferences."
