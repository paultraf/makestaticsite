...
wget_mirror_options=(--recursive -N --level=inf --no-remove-listing) # Basic Wget mirroring options
wget_further_options=(--convert-links --adjust-extension --page-requisites --tries=3) # Wget options that further process the output and make more it more complete
wget_reject_regex=".*login.*|.*logout.*" # URL paths to exclude (regular expressions delimited by pipe).
