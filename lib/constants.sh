...
web_source_exclude_dirs=        # comma-separate list of directories to exclude (relative to working mirror directory) - note: this ought to be refined to be extension dependent
asset_extensions="js,css,jpeg,jpg,gif,png,pdf,doc,docx,odt,ppt,xls,xlsx,svg,heic,webp,mp3,m4a,ogg,wav,avi,mpg,mp4,mov,ogv,wmv,3gp,3gp2,cff,ttf,eot,woff,woff2,map"  # list of file extensions for assets that may be retrieved by Wget in phase 3.  If no extensions are defined, then cURL will be used to remove non-HTML assets, but all other assets will be accepted
