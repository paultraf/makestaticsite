...
# shellcheck source=lib/mod_wayback.sh
  ...
  if [ "$quota" = "on" ] || [ "$quota" = "files" ] || [ "$quota" = "requests" ]; then
    ...
    if [ "$quota" = "on" ] || [ "$quota" = "requests" ]; then
      if [ "$wayback_url" = "yes" ]; then
        echolog -n " $quota_requests_wayback" 
      else
        echolog -n " $quota_requests" 
      fi
      echolog " Wget requests."
    fi
    echolog "(Settings may be changed in constants.sh)" 
  fi
...
# shellcheck disable=SC2034
  ...
  if (( wget_threads > 1 )); then
  ...
  elif [ "$quota" = "on" ] || [ "$quota" = "files" ] || [ "$quota" = "requests" ]; then
    while read -r input_url; do
      ...
      if [ "$quota" = "on" ] || [ "$quota" = "requests" ]; then
        (( requests_count++ ))
        if { (( requests_count > quota_requests )) || { [ "$wayback_url" = "yes" ] && (( requests_count > quota_requests_wayback )); }; }; then
          # Requests quota exceeded, so break out of loop
          echolog "Have reached requests quota, moving directly to postprocessing."
          (( wget_extra_urls_count = wget_extra_urls_depth + 1 ))
          break
        fi
      fi
...
augment_mirror() {
  if [ "$wget_extra_urls" = "yes" ]; then
    (( requests_count=0 )) # counter for number of calls to Wget 
...
site_postprocessing() {
  ...
  # Rename HTML index files generated by Wget that have MIME type other than text/html
  # Scan for non-HTML MIME types in .html files and change file extensions accordingly
  webpages_default=assets_count=0
  while IFS= read -r line; do webpages_default+=("$line"); done <<<"$(find "$working_mirror_dir" -type f -name  "$wget_default_page" "${asset_exclude_dirs[@]}" -print)"
  (( dir_cut_depth=wayback_snapshot_path_depth+2 ))
  for opt in "${webpages_default[@]}"; do
    if media_file_ext="$(get_mime_type_extension "$opt")" && [[ ! "$media_file_ext" =~ "htm" ]] ; then
      (( assets_count++ ))
      opt_path_dir=${opt%/*}"/"
      mv "$opt" "$opt_path_dir$media_file_stem$assets_count.$media_file_ext"
      opt_relpath=${opt//"$working_mirror_dir"/}
      if [ "$wayback_url" = "yes" ]; then
        opt_relpath=$(echo "$opt_relpath" | cut -f"$dir_cut_depth"- -d'/')
      fi
      opt_relpath_dir=${opt_relpath%/*}"/"
      for opt2 in "${webpages_default[@]}"; do
        if [ -f "$opt2" ]; then
          # case 1: ending /index.html
          sed_subs1=('s|\('"$opt_relpath_dir"'\)'"$wget_default_page"'|'"\1$media_file_stem$assets_count.$media_file_ext"'|g' "$opt2")
          # case 2: ending /        
          sed_subs2=('s|\('"$opt_relpath_dir"'\)\('"[\'\"[:space:]]\)|\1$media_file_stem$assets_count.$media_file_ext\2"'|g' "$opt2") 
          sed "${sed_options[@]}" "${sed_subs1[@]}"
          sed "${sed_options[@]}" "${sed_subs2[@]}"
        fi
      done
